...
(function($) {
	...
	var Drum = function(element, options, transformProp)
	{
		...
		//niko
		settings.htmlselect = HTMLselect;
		...
		var updateData = function() {
			settings.data = [];
			for (var i=0; i<settings.htmlselect.children.length; i++) {
				//console.log("update data:" +$(settings.htmlselect.children[i]).text());
				settings.data.push($(settings.htmlselect.children[i]).text());
			}
	...
	var methods = {
		...
		updateData: function() {
			if ($(this).data('drum'))
				return $(this).data('drum').updateData();
			return false;
		},
